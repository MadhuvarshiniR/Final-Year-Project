...
def store_gesture(gesture):
    gesture_name = gesture.get().lower()
    path = os.getcwd()+'\\gestures\\train\\'+gesture_name
    if not os.path.isdir(path):
        os.mkdir(path)
        os.mkdir(os.getcwd()+'\\gestures\\test\\'+gesture_name)
    else:
            ...
            if hand is not None:
                ...
                if num_imgs_taken <= 300:
                    ...
                    if num_imgs_taken<40:
                        cv2.imwrite(os.getcwd()+'\\gestures\\test\\'+gesture_name+"\\" +str(num_imgs_taken) + '.jpg', thresholded)
...
Header = Label(app, text='Please Enter The Gesture',font=("Comic Sans Ms",18))
...
button = Button(app, text='Start Capturing',width=29,command=store_gesture,font=("Comic Sans Ms",14))
